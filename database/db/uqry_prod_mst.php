...
if(isset($_POST['btneprod']) && ($_POST['btneprod'] != "") && isset($_POST['txtsku']) && ($_POST['txtsku'] != "") && isset($_POST['txtname']) && ($_POST['txtname'] != "") && isset($_POST['lstprodmcat']) && ($_POST['lstprodmcat'] != "") && isset($_POST['lstprodcat']) && ($_POST['lstprodcat'] != "") && isset($_POST['txtprior']) && ($_POST['txtprior'] != ""))
{
  ...
  $sts = glb_func_chkvl($_POST['lststs']);
	...
	$edtvar = glb_func_chkvl($_POST['edtvar']);
	/* if ($prodvariant == 'no' && $edtvar == 'no')
	{
		$prodvariant_ip = 'no';
	}
	else
		...
		if($ursprod_mst == true)
		{
			if ($edtvar == 'no')
			{
				if ($prodvariant == 'no')
				{
					...
					$uqry_prodprc_mst = "UPDATE prodprc_mst set prodprcm_sku='$stl_sku', prodprcm_cstprc = '$stl_cstprc',prodprcm_sleprc='$stl_sleprc',prodprcm_ofrprc='$stl_ofrprc',prodprcm_stkqty='$stl_stkqty',prodprcm_moq='$stl_moq',prodprcm_wt='$stl_wt', prodprcm_mdyon='$dt', prodprcm_mdfyby='$ses_admin' where prodprcm_prodm_id = '$id'";
					$ursprodprc_mst = mysqli_query($conn, $uqry_prodprc_mst);
				}
			}
			else
			{
				$uqry_prod_vrnt = "UPDATE prod_mst SET prodm_vrnttyp = '$prodvar' where prodm_id = '$id'";
				$ursprodprc_mst1 = mysqli_query($conn, $uqry_prod_vrnt);
				...
				else
				{
					...
					if ($irsprodsz_vrtns_mst == true) {
						$vrntszd_ins_id = mysqli_insert_id($conn);
						$stl_sku = $_POST['txtsku'];
						$stl_cstprc = $_POST['txtcstprcsng'];
						$stl_sleprc = $_POST['txtsleprcsng'];
						$stl_ofrprc = $_POST['txtofrprcsng'];
						$stl_stkqty = $_POST['txtstkqtysng'];
						$stl_moq = $_POST['txtmoqsng'];
						$stl_wt = $_POST['txtprodwtsng'];
						$iqry_prodprc_mst = "INSERT into prodprc_mst(prodprcm_prodm_id,prodprcm_vrtn_id, prodprcm_sku, prodprcm_cstprc, prodprcm_sleprc, prodprcm_ofrprc, prodprcm_stkqty, prodprcm_moq, prodprcm_wt, prodprcm_crton, prodprcm_crtby) values ('$id','$vrntszd_ins_id','$stl_sku','$stl_cstprc','$stl_sleprc','$stl_ofrprc','$stl_stkqty','$stl_moq', '$stl_wt', '$dt','$ses_admin')";
						$irsprodprc_mst = mysqli_query($conn, $iqry_prodprc_mst);
					}
				}
